...
| D-021 | 26/07/2026 | Fin d'une inscription | `student_enrollments.end_reason` utilise `YEAR_COMPLETED`, `CLASS_CHANGE` ou `LEFT_SCHOOL` afin de conserver la raison de fin et l'historique des classes. | 🟩 Validée |
...
| D-023 | 29/07/2026 | Génération des matricules | Le matricule conserve sept caractères : un préfixe de rôle (`a`, `e`, `u` ou `p`), un code de date UTC sur trois chiffres (`date.toordinal() modulo 1000`) et un code d'heure UTC sur trois chiffres (`secondes écoulées depuis minuit modulo 1000`). La génération est protégée par un verrou transactionnel propre au rôle. Si le matricule existe déjà, le code horaire est incrémenté jusqu'à trouver une valeur libre. La contrainte d'unicité PostgreSQL reste la protection finale. | 🟩 Validée |
