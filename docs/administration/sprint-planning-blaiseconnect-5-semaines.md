...
| US-015 | 14/08/2026 | Calcul des moyennes | En tant qu’utilisateur autorisé, je veux que les moyennes soient calculées automatiquement afin d’obtenir des résultats fiables par matière et par période. | 🟨 En cours | Calculs backend dans `backend/app/services/academic_calculation_service.py`, `assessment_service.py` et `subject_service.py` ; formule d’arrondi définitive encore à valider |
